...
public class SpawnTask extends BukkitRunnable {
    ...
    public void spawnMerchant() {
        ...
        if (world == null) {
            Bukkit.getLogger().warning("Мир 'world' не найден!");
            return;
        }

        var players = Bukkit.getOnlinePlayers();
        if (players.isEmpty()) {
            Bukkit.getLogger().warning("Нет онлайн игроков!");
            return;
        }

        // Read configurable radius / attempts
        ...
        // Пробуем найти место рядом с каждым игроком
        for (Player player : players) {
            Location spawnLocation = findSafeLocationNearPlayer(player.getLocation(), radius, attempts);
            if (spawnLocation != null) {
                spawnMerchantAtLocation(world, spawnLocation);
                return;
            }
            // Если не найдено идеальное место, ищем наименее плохое
            ...
            if (score > fallbackScore && bestEffort != null) {
                fallbackLocation = bestEffort;
                fallbackScore = score;
            }
        }

        if (fallbackLocation != null) {
            Bukkit.getLogger().warning("Не найдено полностью безопасное место, спавним в наиболее подходящем!");
            spawnMerchantAtLocation(world, fallbackLocation);
            return;
        }

        Bukkit.getLogger().warning("Не найдено безопасное место для спавна рядом ни с одним игроком!");
    }

    private boolean isLocationInRiverRegion(Location loc) {
        // Пример: если Y < 65 и X/Z в определённых границах, считаем "ривер".
        // Замените на вашу логику, если есть WorldGuard или другой плагин регионов.
        int y = loc.getBlockY();
        int x = loc.getBlockX();
        int z = loc.getBlockZ();
        // Примерные координаты для river (замените на реальные, если нужно)
        if (y < 65 && x > -200 && x < 200 && z > -200 && z < 200) {
            return true;
        }
        return false;
    }
    ...
    private Location findSafeLocationNearPlayer(Location center, int radius, int attempts) {
        ...
        for (int attempt = 0; attempt < attempts; attempt++) {
            ...
            int y = groundY + 1; // ставим ноги на блок выше, чтобы не встраивался
            Location testLoc = new Location(world, x + 0.5, y, z + 0.5);
            if (isLocationSafe(testLoc) && !isLocationInRiverRegion(testLoc)) {
                return testLoc;
            }
        }
        return null;
    ...
    private Location findBestEffortLocationNearPlayer(Location center, int radius, int attempts) {
        ...
        for (int attempt = 0; attempt < attempts; attempt++) {
            ...
            if (!isLocationInRiverRegion(testLoc) && score > bestScore) {
                bestScore = score;
                bestLoc = testLoc;
            }
        }
    ...
    private int evaluateLocationSafety(Location loc) {
        try {
            ...
            if (SAFE_GROUND_BLOCKS.contains(ground.getType()) && ground.getType().isSolid()) {
                score += 3;
            ...
            if (!UNSAFE_BLOCKS.contains(feet.getType())) {
                score += 1;
            }
            Block head = world.getBlockAt(x, y + 1, z);
            if (head.getType().isAir() || head.isPassable()) {
                score += 2;
            }
            if (!UNSAFE_BLOCKS.contains(head.getType())) {
                score += 1;
            }
            if (feet.getLightLevel() >= 5) {
                score += 2;
            } else if (feet.getLightLevel() >= 1) {
                score += 1;
            }
            return score;
        } catch (Exception e) {
    ...
    private boolean isLocationSafe(Location loc) {
        try {
            ...
            Block ground = world.getBlockAt(x, y - 1, z);
            if (!SAFE_GROUND_BLOCKS.contains(ground.getType()) ||
            ...
            Block feet = world.getBlockAt(x, y, z);
            if (UNSAFE_BLOCKS.contains(feet.getType()) ||
                    (!feet.getType().isAir() && !feet.isPassable())) {
                return false;
            }
            ...
            // Проверяем освещенность (не в полной темноте)
            if (feet.getLightLevel() < 5) {
                return false;
            }
    ...
    private void spawnMerchantAtLocation(World world, Location loc) {
        try {
            ...
            broadcastSpawnMessage(loc, despawnDurationSeconds);
            playGlobalSpawnSounds();
            setupBossBar(loc, despawnDurationSeconds);
            startParticleTask();

            Bukkit.getLogger().info("Торговец заспавнен на: " + loc.getBlockX() + " " + loc.getBlockY() + " " + loc.getBlockZ());
        } catch (Exception e) {
            Bukkit.getLogger().warning("Ошибка спавна торговца: " + e.getMessage());
        }
    }
    ...
    private void startParticleTask() {
        ...
        final String typeStr = cfg.getString("particles.type", "ENCHANTMENT_TABLE");
        final int interval = cfg.getInt("particles.interval-ticks", 10);
        final int count = Math.max(1, cfg.getInt("particles.count", 8));
        final double radius = cfg.getDouble("particles.radius", 0.6D);
        Particle parsed;
        try { parsed = Particle.valueOf(typeStr.toUpperCase()); } catch (Exception ex) { parsed = Particle.ENCHANTMENT_TABLE; }
        final Particle particle = parsed;
        particleTask = new BukkitRunnable() {
            double angle = 0;
            @Override
            public void run() {
                if (currentMerchant == null || currentMerchant.isDead()) { cancel(); return; }
                Location base = currentMerchant.getLocation().clone().add(0, 1.0, 0);
                for (int i = 0; i < count; i++) {
                    double a = angle + (2 * Math.PI / count) * i;
                    double x = Math.cos(a) * radius;
                    double z = Math.sin(a) * radius;
                    base.getWorld().spawnParticle(particle, base.getX() + x, base.getY() + 0.1 * Math.sin(angle + i), base.getZ() + z, 1, 0,0,0,0);
                }
                angle += Math.PI / 20;
            }
        }.runTaskTimer(MysteryMerchant.getInstance(), 0L, Math.max(1L, interval));
    }

    private void broadcastSpawnMessage(Location loc, long despawnSeconds) {
        var cfg = MysteryMerchant.getInstance().getConfig();
        String template = cfg.getString("spawn-message", "§6§lТаинственный Торговец появился на координатах §e[%x; %y; %z]§f! §7Он исчезнет через %time%.");
            ...
            for (String line : lines) {
                ...
                if (m.find()) {
                    if (p.isOp() || p.hasPermission("mysterymerchant.tp")) {
                        ...
                        String coordsText = line.substring(m.start(), m.end());
                        ...
                        coordsComp.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(ColorUtil.color("§eКликните, чтобы телепортироваться")) .create()));
                        TextComponent afterComp = new TextComponent(after);
                        TextComponent full = new TextComponent("");
                        full.addExtra(beforeComp);
                        full.addExtra(coordsComp);
                        full.addExtra(afterComp);
                        p.spigot().sendMessage(full);
                    } else {
                        p.sendMessage(line); // not clickable for non-op
                    }
                } else {
                    p.sendMessage(line);
                }
    ...
    private void playGlobalSpawnSounds() {
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            if (section != null) {
                for (String key : section.getKeys(false)) { // but our config uses list - handle both list/section
                    ...
                    try { p.playSound(p.getLocation(), Sound.valueOf(soundName), volume, pitch); } catch (Exception ignored) {}
                }
            } else {
                // fallback to list variant
                ...
                if (list != null) {
                    ...
                    // ultimate fallback: default trio
    ...
    private void setupBossBar(Location loc, long despawnSeconds) {
        removeBossBar();
        merchantBar = Bukkit.createBossBar("§6Таинственный торговец загружается...", BarColor.YELLOW, BarStyle.SEGMENTED_12);
        merchantBar.setProgress(1.0);
        for (Player p : Bukkit.getOnlinePlayers()) merchantBar.addPlayer(p);
    ...
    private void startCountdown(Location loc, long totalSeconds) {
        ...
        countdownTask = new BukkitRunnable() {
            @Override
            public void run() {
                if (currentMerchant == null || currentMerchant.isDead()) { cancel(); removeBossBar(); return; }
                long remaining = (despawnAtMillis - System.currentTimeMillis()) / 1000L;
                if (remaining <= 0) {
                    removeMerchant();
                    Bukkit.broadcastMessage(ColorUtil.color("§cТаинственный торговец исчез!"));
                    cancel();
                    return;
                }
                String timeStr = formatTime(remaining);
                double progress = Math.max(0.0, Math.min(1.0, (double) remaining / totalSeconds));
                String title = "§6Таинственный торговец §7исчезнет через §e" + timeStr +
                        " §7| §fX:§a" + loc.getBlockX() + " §fY:§a" + loc.getBlockY() + " §fZ:§a" + loc.getBlockZ();
                merchantBar.setTitle(ColorUtil.color(title));
                merchantBar.setProgress(progress);
            }
        }.runTaskTimer(MysteryMerchant.getInstance(), 0L, 20L);
    }
    ...
    public void removeMerchant() {
        if (currentMerchant != null) { try { currentMerchant.remove(); } catch (Exception ignored) {} currentMerchant = null; }
