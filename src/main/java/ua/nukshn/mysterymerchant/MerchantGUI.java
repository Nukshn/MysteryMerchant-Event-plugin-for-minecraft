...
public class MerchantGUI {
    ...
    private static String guiTitle() {
        return ColorUtil.color(MysteryMerchant.getInstance().getConfig().getString("gui-title", "§6Магазин Торговца"));
    }
    ...
    private static void addPricedItem(Inventory inv, int slot, String itemId, Material mat, String displayName, List<String> baseLore, int price, int limit, Player viewer) {
        ...
        lore.add(ColorUtil.color("&8Цена: &6" + price + "&e$"));
        if (limit >= 0) {
            int remaining = MysteryMerchant.getInstance().getPurchaseManager().remaining(viewer.getUniqueId(), itemId, limit);
            lore.add(ColorUtil.color("&7Лимит: &e" + remaining + "&7/&6" + limit));
        } else lore.add(ColorUtil.color("&7Лимит: &a∞"));
        lore.add(ColorUtil.color("&7Нажмите, чтобы купить"));
        meta.setLore(lore);
    ...
    public static boolean isMerchantItem(ItemStack stack) {
        if (stack == null || !stack.hasItemMeta()) return false;
        ItemMeta meta = stack.getItemMeta();
        return meta.getPersistentDataContainer().has(MERCHANT_ITEM_KEY, PersistentDataType.BYTE);
    }

    public static int getPrice(ItemStack stack) {
        if (stack == null || !stack.hasItemMeta()) return -1;
        ItemMeta meta = stack.getItemMeta();
        Integer p = meta.getPersistentDataContainer().get(PRICE_KEY, PersistentDataType.INTEGER);
        return p == null ? -1 : p;
    }

    public static String getItemId(ItemStack stack) {
        if (stack == null || !stack.hasItemMeta()) return null;
        return stack.getItemMeta().getPersistentDataContainer().get(ITEM_ID_KEY, PersistentDataType.STRING);
    }

    public static int getLimit(ItemStack stack) {
        if (stack == null || !stack.hasItemMeta()) return -1;
        Integer v = stack.getItemMeta().getPersistentDataContainer().get(LIMIT_KEY, PersistentDataType.INTEGER);
        return v == null ? -1 : v;
    }
    ...
    private static void setupItems(Inventory gui, Player viewer) {
        ...
        int[] preferredSlots = {11,13,15,10,16,12,14};
        int index = 0;
        for (String id : section.getKeys(false)) {
            ConfigurationSection it = section.getConfigurationSection(id);
            if (it == null) continue;
            String matStr = it.getString("material", "STONE");
            Material mat = Material.matchMaterial(matStr.toUpperCase());
            if (mat == null) mat = Material.STONE;
            int price = it.getInt("price", 0);
            int limit = it.getInt("limit", -1);
            String name = it.getString("name", id);
            List<String> rawLore = it.getStringList("lore");
            List<String> lore = new ArrayList<>(rawLore);
            int slot = index < preferredSlots.length ? preferredSlots[index] : index;
            addPricedItem(gui, slot, id, mat, name, lore, price, limit, viewer);
            index++;
        }
    }

    public static boolean isGuiTitle(String title) {
        return guiTitle().equals(title);
    }
}